# Distributing Double Copy Paste
...
3. Find your Team ID for the paid account: `security find-identity -v -p codesigning` - look for the "Developer ID Application: ..." entry and note
   the ID in parentheses. It may differ from the free-tier
...
xcodebuild -project DoubleCopyPaste.xcodeproj -scheme DoubleCopyPaste \
  -configuration Release archive -archivePath build/DoubleCopyPaste.xcarchive

# Export the signed .app from the archive
xcodebuild -exportArchive -archivePath build/DoubleCopyPaste.xcarchive \
...
ditto -c -k --keepParent "build/export/Double Copy Paste.app" build/DoubleCopyPaste.zip
xcrun notarytool submit build/DoubleCopyPaste.zip \
  --keychain-profile "notarytool-profile" --wait

# Staple the notarization ticket to the .app itself (so it works offline too)
xcrun stapler staple "build/export/Double Copy Paste.app"

# Verify Gatekeeper is satisfied
spctl -a -vvv -t install "build/export/Double Copy Paste.app"
```
...
```sh
ditto -c -k --keepParent "build/export/Double Copy Paste.app" "DoubleCopyPaste-0.1.0.zip"
```
